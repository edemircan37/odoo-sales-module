...
class SaleOrder(models.Model):
    ...
    salesperson_note = fields.Text(string="Satış Temsilci Notu")

    x_customer_note = fields.Char(string="Müşteri Notu")

    x_priority_level = fields.Selection([
        ('low', 'Düşük'),
        ('medium', 'Orta'),
        ('high', 'Yüksek'),
    ], string="Öncelik", default='medium')

    x_confirmed = fields.Boolean(string="Onaylandı mı?")

    def action_simple_button(self):
        for order in self:
            raise UserError(f'Sipariş {order.name} için buton tıklandı!')
